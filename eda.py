...
def perform_eda(df):
    ...
    df['ActivityHour'] = df['ActivityDateTime'].dt.hour

    # Plot the distribution of activity hours
    plt.figure(figsize=(10, 6))
    sns.histplot(df['ActivityHour'], bins=24, kde=True)
    plt.title('Distribution of Activities by Hour of the Day')
    plt.xlabel('Hour of the Day')
    plt.ylabel('Frequency')
    plt.xticks(range(24))  # Setting x-ticks for each hour
    plt.show()
